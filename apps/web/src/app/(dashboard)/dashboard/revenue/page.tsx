...
export default function AdminRevenuePage(): React.ReactElement {
  ...
  const TX_LIMIT = 15;
  // Track which quick-range button is currently active (-1 = custom dates)
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
          ...
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <div className="flex items-center gap-2">
              <input
                type="date"
                value={dateFrom}
                onChange={e => { setDateFrom(e.target.value); setActiveQuickRange(0); setTxPage(1); }}
              ...
              <input
                type="date"
                value={dateTo}
                onChange={e => { setDateTo(e.target.value); setActiveQuickRange(0); setTxPage(1); }}
                className="text-sm border border-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary/30"
              />
            </div>
      ...
      {/* Revenue active-filter bar */}
      {(salonId || method || txSearch) && (() => {
        const cnt = [salonId, method, txSearch].filter(Boolean).length;
        return (
          <div className="px-4 py-2 bg-amber-50 border border-amber-100 rounded-xl flex items-center justify-between animate-in slide-in-from-top-1 duration-200">
            <div className="flex items-center gap-2">
              <svg className="w-4 h-4 text-amber-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2a1 1 0 01-.293.707L13 13.414V19a1 1 0 01-.553.894l-4 2A1 1 0 017 21v-7.586L3.293 6.707A1 1 0 013 6V4z" />
              </svg>
              <span className="text-xs font-semibold text-amber-700">
                Đang lọc với <span className="bg-amber-200 text-amber-800 rounded-full px-1.5 py-0.5 text-[10px] font-bold">{cnt}</span> điều kiện
              </span>
            </div>
            <button
              onClick={() => { setSalonId(''); setMethod(''); setTxSearch(''); setTxPage(1); }}
