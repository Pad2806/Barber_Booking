...
export default function BarberBookingsPage() {
  ...
  // Debounce search 500ms to avoid re-fetching on every keystroke
  useEffect(() => {
  ...
  const { data: response, isLoading } = useQuery({
    queryKey: ['bookings', 'barber-dashboard', me?.staff?.id, debouncedSearch, statusFilter, page, limit, dateFrom, dateTo],
    queryFn: () =>
      bookingApi.getAll({
        staffId: me?.staff?.id,
  ...
  // bookings directly from server (server-side status filter)
  ...
  // On re-fetches (e.g. search/filter changes), keep showing existing data
