...
export default function AdminServicesPage() {
  ...
  const activeFilterCount = [
    debouncedSearch, category, isActive !== 'ALL' ? isActive : '', minPrice, maxPrice
  ].filter(Boolean).length;
  const hasActiveFilters = activeFilterCount > 0;
  const clearAllServiceFilters = () => {
    setSearch(''); setCategory(undefined); setIsActive('ALL'); setMinPrice(''); setMaxPrice(''); setPage(1);
  };
      ...
      <Card className="border-none shadow-sm bg-slate-50/50">
        ...
        {hasActiveFilters && (
          <div className="px-6 py-2 bg-amber-50 border-b border-amber-100 flex items-center justify-between animate-in slide-in-from-top-1 duration-200">
            ...
            <button onClick={clearAllServiceFilters}
