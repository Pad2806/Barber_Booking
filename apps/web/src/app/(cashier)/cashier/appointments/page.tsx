    ...
    return (
      <div className="flex bg-white rounded-[2rem] border border-slate-100 items-center justify-center min-h-[500px]">
        <div className="flex flex-col items-center gap-4">
           ...
           <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest italic">Syncing Calendar...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
           <div>
              <h1 className="text-4xl font-black text-slate-900 tracking-tighter italic uppercase">
                 Service <span className="text-[#C8A97E]">Agenda</span>
              </h1>
              <p className="text-slate-500 font-medium font-serif italic text-sm">Quản lý toàn bộ lịch hẹn tại chi nhánh trong tầm tay.</p>
           </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {bookings?.length === 0 ? (
          <div className="bg-white rounded-[2rem] p-24 flex flex-col items-center justify-center border-2 border-dashed border-slate-100 shadow-sm">
             <div className="p-8 rounded-full bg-slate-50 text-slate-200 mb-6">
                <Calendar className="w-16 h-16" />
             </div>
             <p className="text-slate-400 font-black italic uppercase text-xs tracking-widest">No matching appointments found</p>
          </div>
        ) : (
          bookings?.map((booking: any) => (
            <Card key={booking.id} className="group border-none shadow-lg hover:shadow-2xl transition-all duration-500 rounded-[2.5rem] bg-white overflow-hidden">
               <CardContent className="p-0">
                  <div className="flex flex-col lg:flex-row items-stretch">
                     {/* Time Side */}
                     <div className="lg:w-48 bg-slate-50 p-8 flex lg:flex-col items-center justify-center lg:border-r border-slate-100 text-center gap-4">
                     ...
                     <div className="flex-1 p-8 flex flex-col md:flex-row md:items-center justify-between gap-8">
                        <div className="flex items-center gap-6">
                           ...
                           <div className="space-y-2">
                              ...
                              <div className="flex flex-wrap gap-2 text-slate-400">
                                 ...
                                 <span className="text-xs font-bold leading-none">{booking.customerPhone || 'N/A'}</span>
                              </div>
                           </div>
                        </div>

                        <div className="flex items-center gap-4 bg-slate-50 px-6 py-4 rounded-3xl border border-slate-100 min-w-[220px]">
                           ...
                           <div>
                              <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] mb-0.5 italic">Assigned Tech</p>
                              <p className="text-xs font-black text-slate-900 italic uppercase">
                                 {booking.staff?.user?.name || 'Open Selection'}
                              </p>
                           </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                           <Button 
                             ...
                             className="bg-slate-900 hover:bg-slate-800 text-white rounded-2xl font-black italic uppercase text-[10px] tracking-widest h-12 px-6 shadow-xl transition-all hover:scale-105"
                           >
                              Details
                           </Button>
