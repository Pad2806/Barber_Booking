...
export default function PaymentPage() {
  ...
  // ── Animate in on mount (page-as-modal effect) ───────────────
  const [mounted, setMounted] = useState(false);
  useEffect(() => {
    // Lock body scroll (modal feel)
  ...
  // ── Countdown → auto-cancel on timeout ───────────────────────
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setCountdown(prev => {
        if (prev <= 1) {
          ...
          bookingApi
            .cancel(bookingId, 'Hết thời gian thanh toán cọc')
  ...
  // ════════════════════════════════════════════════════════════
  //  OUTER WRAPPER — backdrop + slide animation
  // ════════════════════════════════════════════════════════════
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
      ...
      <div
        ...
        onClick={paymentStatus === 'PAID' ? handleClose : undefined}
      />
      ...
      <div
        className={cn(
          ...
          'max-h-[92vh] md:max-h-[85vh] flex flex-col overflow-hidden',
        ...
        {loading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8 gap-5">
            ...
            <div>
              ...
              <p className="text-sm text-[#8B7355] max-w-xs mx-auto">
                Phiên giao dịch đã hết hạn. Thử lại hoặc thanh toán trực tiếp tại salon.
              </p>
            </div>
          ...
          <>
            ...
            <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-[#F0EBE3] shrink-0">
              {/* Drag handle (mobile) */}
              <div className="absolute top-2.5 left-1/2 -translate-x-1/2 w-10 h-1 rounded-full bg-[#E8E0D4] md:hidden" />
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              ...
              {/* BANK TRANSFER TAB */}
              {activeTab === 'bank' && (
                <div className="space-y-0 rounded-2xl border border-[#E8E0D4] overflow-hidden">
                  {[
                    { label: 'Ngân hàng', value: qrData?.bankName || qrData?.bankCode || '—', field: 'bank', className: 'uppercase', noCopy: true },
                  ...
                  ].map(({ label, value, field, className, noCopy, highlight, rawValue }, i, arr) => (
                    <div
                      key={field}
                      className={cn(
                    ...
                    >
                      <div className="min-w-0">
                        <p className={cn(
                          ...
                          highlight ? 'text-[#C8A97E]' : 'text-[#8B7355]'
                        )}>
                          {label}
                        </p>
                        <p className={cn(
                          'text-sm font-bold text-[#2C1E12] truncate',
                          className
                        )}>
                          {value}
                        </p>
                      ...
                      {!noCopy && (
                        <CopyBtn field={field} value={rawValue || value} />
                      )}
                    </div>
                  ))}
                </div>
