...
const MANAGER_MENU_ITEMS = [
  ...
  { key: 'staff', href: '/manager/staff', label: 'Cửa hàng', icon: Users },
  { key: 'schedule', href: '/manager/schedule', label: 'Lịch làm việc', icon: Clock },
];
...
export default function ManagerLayout({ children }: ManagerLayoutProps) {
  ...
  useEffect(() => {
    if (me && me.role !== Role.MANAGER && me.role !== Role.SUPER_ADMIN && me.role !== Role.SALON_OWNER) {
        // Not a managerRole
    }
  }, [me]);
      ...
      <div className={cn("px-6 mb-8 flex items-center", isCollapsed && !isMobileOpen ? "justify-center px-2" : "justify-between")}>
        <Link href="/manager/dashboard" className={cn("font-heading font-black italic tracking-tighter transition-all hover:scale-105", isCollapsed && !isMobileOpen ? "text-xl" : "text-2xl")}>
          {isCollapsed && !isMobileOpen ? (
            <span className="text-[#C8A97E] text-2xl">R</span>
          ) : (
            <span className="text-white uppercase tracking-tight font-black">REETRO<span className="text-[#C8A97E] ml-1">BRANCH</span></span>
          )}
        </Link>
      </div>
                ...
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="flex items-center gap-3 pl-2 pr-1 rounded-full hover:bg-slate-100">
                    <div className="text-right hidden sm:block">
                      ...
                      <p className="text-[10px] text-[#C8A97E] mt-1 font-bold uppercase">{me?.staff?.position || me?.role}</p>
                    </div>
