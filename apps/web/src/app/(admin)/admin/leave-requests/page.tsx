      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col text-left">
            <span className="font-bold text-slate-900 text-sm">{row.original.staff?.user?.name}</span>
            <div className="flex items-center gap-1.5">
              <Store className="w-3 h-3 text-slate-400" />
              <span className="text-[10px] font-medium text-slate-500 uppercase tracking-tight">{row.original.staff?.salon?.name}</span>
            </div>
          </div>
        </div>
      )
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex flex-col gap-0.5 text-left">
            ...
            <span className="text-[10px] text-slate-400 font-medium">Gửi lúc: {dayjs(row.original.createdAt).format('HH:mm DD/MM')}</span>
          </div>
        );
      }
    },
        ...
        return (
          <div className="flex items-center justify-end gap-2">
            {req.status === 'PENDING' ? (
              ...
              <div className="text-[10px] text-rose-400 italic font-medium max-w-[150px] truncate">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
           ...
           <h1 className="text-2xl font-bold text-slate-900 tracking-tight">
              Quản lý <span className="text-[#7C3AED]">Nghỉ phép</span>
           </h1>
           <p className="text-slate-500 text-sm mt-1">Phê duyệt và theo dõi yêu cầu nghỉ phép toàn hệ thống.</p>
        </div>
      ...
      <div className="bg-white border rounded-2xl shadow-sm overflow-hidden">
        <div className="p-4 border-b bg-slate-50/50">
          ...
          <div className="mt-4 flex items-center justify-between">
            <Tabs defaultValue="PENDING" onValueChange={setActiveTab} className="h-9">
              <TabsList className="bg-slate-100 p-1 rounded-lg h-full">
                 <TabsTrigger value="ALL" className="rounded-md px-4 text-[10px] font-bold uppercase tracking-wider data-[state=active]:bg-white data-[state=active]:text-[#7C3AED] shadow-none">TẤT CẢ</TabsTrigger>
                 <TabsTrigger value="PENDING" className="rounded-md px-4 text-[10px] font-bold uppercase tracking-wider data-[state=active]:bg-white data-[state=active]:text-amber-500 shadow-none">CHỜ DUYỆT</TabsTrigger>
                 <TabsTrigger value="APPROVED" className="rounded-md px-4 text-[10px] font-bold uppercase tracking-wider data-[state=active]:bg-white data-[state=active]:text-emerald-500 shadow-none">ĐÃ DUYỆT</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
        </div>
         ...
         <DialogContent className="sm:max-w-[400px] border-none shadow-2xl p-0 overflow-hidden bg-white rounded-2xl">
            <DialogHeader className="p-6 pb-0 text-left">
               ...
               <DialogDescription className="text-sm text-slate-500 pt-1">
                  Lý do từ chối nghỉ phép của <span className="font-bold text-slate-900">{selectedRequest?.staff?.user?.name}</span>?
               </DialogDescription>
            </DialogHeader>

            <div className="p-6 space-y-4">
               <Textarea 
                 placeholder="Nhập lý do (ví dụ: Trùng lịch làm việc, thiếu nhân sự...)"
                 value={rejectReason}
